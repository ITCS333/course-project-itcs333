...
if(strlen($password) < 8){
    http_response_code(400);
    echo json_encode(["success" => false,
                      "message" => "password must be 8 charcters or more"]);
    exit;
}
...
try{
    ...
    // admins table
    $stmt = $pdo->prepare("SELECT id,name,email,password FROM admins
                           WHERE email = ?");
    ...
    if($user){
        $isValidPassword = password_verify($password, $user["password"]) || hash_equals($user["password"], $password);
    }
    ...
    // I put this task before send success messages 
        ...
        // TODO: Encode the error response array as JSON and echo it
