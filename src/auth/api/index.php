...
if ($_SERVER['REQUEST_METHOD'] !== 'POST'){
    // return echo json_encode(["success" => false, "message" => "Invalid request method"]);
    // fixed: cannot use 'return echo' — echo the response and exit
...
if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    // return echo json_encode(["success" => false, "message" => "Invalid email format"]);
    // fixed: echo + exit
...
if (strlen($password) < 8) {
    // return echo json_encode(["success" => false, "message" => "Password must be at least 8 characters"]);
    // fixed: echo + exit
